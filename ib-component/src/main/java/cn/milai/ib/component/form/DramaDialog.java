...
public class DramaDialog extends AbstractTextFormComponent {
	...
	private static final int SPEAKER_IMAGE_WIDTH = 60;
	private static final int SPEAKER_IMAGE_HEIGHT = 60;
	...
	private void render(Graphics g) {
		...
		int nowBottom = verMargin + textHeight + SPEAKER_IMAGE_HEIGHT;
		// 至少绘制一行
	...
	public Image getImage() {
		...
		// 说话者头像
		if (speaker != null) {
			g.drawRect(0, 0, SPEAKER_IMAGE_WIDTH, SPEAKER_IMAGE_HEIGHT);
			g.drawImage(speaker, 0, 0, SPEAKER_IMAGE_WIDTH, SPEAKER_IMAGE_HEIGHT, null);
		}
		// 表示还有下一页的箭头
		if (readIndex < text.length()) {
