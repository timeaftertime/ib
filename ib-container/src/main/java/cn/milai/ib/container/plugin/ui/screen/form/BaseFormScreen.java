...
import cn.milai.ib.container.plugin.ui.screen.form.bar.TitleBar;
...
public class BaseFormScreen extends BaseScreen implements FormScreen {
	...
	public void onInit() {
		...
		titleBar.minimizeButton().onPressUp(e -> form.setExtendedState(JFrame.ICONIFIED));
		titleBar.closeButton().onPressUp(e -> ui().containers().forEach(LifecycleContainer::close));
		form = new BarForm() {
			@Override
