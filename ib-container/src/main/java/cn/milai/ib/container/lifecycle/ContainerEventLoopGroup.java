...
/**
 * {@link ContainerEventLoop} 组
 * @author milai
 * @date 2022.04.16
 */
public class ContainerEventLoopGroup extends MultithreadEventExecutorGroup implements EventExecutorGroup {

	/**
	 * 创建一个使用默认 {@link Executor} 的事件循环
	 * @param size
	 */
	public ContainerEventLoopGroup(int size) {
		this(size, null, null);
	}
	 ...
	 */
	public ContainerEventLoopGroup(int size, Executor executor, Executor eventExecutor) {
		this(size, executor, eventExecutor, s -> new ConcurrentLinkedQueue<>(), RejectedExecutionHandlers.reject());
	}

	/**
	 * 创建一个 {@link LifecycleContainer} 事件循环
	...
	protected EventExecutor newChild(Executor executor, Object... args) throws Exception {
		...
		return new ContainerEventLoop(this, executor, eventExecutor, queueFactory, rejectedHandler);
	}

	@Override
	public ContainerEventLoop next() {
		return (ContainerEventLoop) super.next();
	}
